...
tetraloop,  are  also   arranged  sequentially  (i.e.   are  connected
...
closing  the  tetraloops  are  isosteric to  the  G$\cdot$A  base-pair
\cite{lemieux2006}.

Some   studies   have  used   molecular   dynamics   to  explore   the
conformational space  of the  GNRA tetraloop, and  they find a  set of
conformational states  for the GNRA  motif which are later  related to
existing X-Ray and NMR structures available at the protein data
...
can be expanded  to include various known motifs  the users would wish
...
program  takes  22.1 seconds  to  download and  analyze  the  whole
structure in an Intel Core Duo of 2.13GHz with 8Gb of RAM. 
...
computing server, such task is quite simple using getMotif.
...
each  of these motifs  we arrange  their average  values in  a 3  by 6
matrix $S$ where each row corresponds to one of the three steps in the
tetraloop as show in Table \ref{tab:seed}.
...
\hline
\end{tabular}
\caption{Average base  step parameter values for 20  GNRA motifs found
  in the large subunit of the ribosome PDBID:1ffk.}
\label{tab:seed}
\end{center}
\end{table}

We  then  compute  a  score  to determine  the  distance  between  all
sequences formed by  three sequential steps in the  RNA structure, and
the GNRA  motif seed. The score is  simply the sum of  the elements of
the  difference matrix $X_{k}$,  where the  difference is  between the
GNRA matrix $S$ and each one of the $k-3$ sequential 3 by six matrices
...
\begin{figure}
\centering 
\includegraphics[angle=0, scale=1]{Chapter5/gnrahisto.png}
...
Using our program  in the list of 355 of RNA  strucutures given by the
ROC, and  the selected cutoff value we  find that 60 out  of these 355
structures have  at least  one GNRA 3D  motif in  them. A list  of all
identified motifs in these 60 structures can be seen in Table such.
