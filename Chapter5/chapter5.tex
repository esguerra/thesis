...
and  the selected cutoff  value of  150 we  find in  7 minutes  and 29
seconds that 75  out of these 355 structures  are identified as having
at least one GNRA 3D motif,  and a total of 211 GNRA Motif candidates.
The list of  all identified motifs in these 75  structures in shown in
Table \ref{tab:gnrascores}, where  an additional check for consistency
is that  of having zero  endocyclic overlap in  the first step  of the
...
GNRA tetraloop motif candidates, which  are all in fact tetraloops, 14
of  them conform  to the  GNRA  sequence, 5  start with  UN, one  with
A. When  aligned using the reference  frame of the first  two bases we
see  that  they   belong  to  two  main  groups.    Which  are  mainly
differentiated by the rise of the last step.

The first step of the  identified candidates is shown in a scatterplot
\ref{fig:scattergnra} of step-parameter  values, where it's clear that
they  constitute  a well  defined  region  of  the space  of  possible
rigid-body parameter configuration space  for the large subunit of the
ribosome.
