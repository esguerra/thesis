...
Our analysis  computed the validation scores for  a number of
clusters  ranging from  $k=2$, up  to $k=80$  clusters,  and evaluated
hierarchical  methods (hierarchical,  diana), and  partitional methods
(kmeans, pam, som, sota).  The connectivity measure must be minimized,
and the  average silhouette width  (silhoutte) and dunn index  must be
maximized.  With this  in  mind, we  see  that the  method labeled  as
...
distance (AD), and the average distance between means (ADM), again the
details of  such measures are given  in the Appendix~\ref{appendix_a}.
...
\begin{figure}
 \centering
\includegraphics[angle=0, scale=0.4]{Chapter2/STval_sta.png}
...
\cite{arnott1973} work which are provided in 
table~\ref{tab:conformations}  along  with  other  standard  base-step
parameters like those of A-DNA, and B-DNA conformations.
...
The cluster validation analysis has been repeated for a filtered 


With  the filtered dataset,  which we  will refer  to as  the non-ARNA
dataset, we  have performed a  cluster validation analysis  using only
the hierarchical methods, that is, the agglomerative clustering method
(labeled  hierarchical  in  Figure~\ref{fig:noarna}), and  the  divise
clustering method  (labeled as diana  in Figure~\ref{fig:nonarna}). In
Figure~\ref{fig:noarna} we  have plotted  the results of  the internal
validation scores and we can  see that the connectivity and silhouette
measures still  give us an optimal  number of cluster  of two, whereas
the dunn index tells us that the optimal number of clusters is 64.  We
find it  interesting that for the case  of the dunn index  we see that
this measure becomes constant at values of 17, 30, 56, and 64.
